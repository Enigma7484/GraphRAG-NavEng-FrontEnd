...
function inferBadges(route) {
  ...
  if (route.safety_score >= 0) badges.push({ label: "Safer", tone: "blue" });
...
export default function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`${API_BASE}/rank-routes`, {
        origin,
        destination,
        preference,
        dist_meters: 6000,
      ...
      setError(
        err?.response?.data?.detail ||
          "Failed to fetch ranked routes. Make sure the FastAPI backend is running."
      );
    } finally {
      setLoading(false);
    }
  ...
  const explanation = useMemo(() => {
    ...
    if (selectedRoute.safety_score >= 0) reasons.push("better safety proxy");
    ...
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand-block">
          ...
          <p>
            A Toronto-focused geospatial ML demo for ranking walking routes by
        ...
        <div className="panel">
          ...
          {!loading && routes.length === 0 && (
            <div className="empty-box">
              Submit an origin, destination, and route preference to see ranked
              walking routes.
            </div>
          )}
      ...
      <main className="main-content">
        <div className="topbar">
          <div>
            ...
            <p>
              Compare alternative routes and inspect why the selected route was
              ranked highly.
            </p>
          </div>
            ...
            {!selectedRoute ? (
              ...
              <>
                ...
                <div className="metric-grid">
                  ...
                  <Metric label="Score" value={selectedRoute.score.toFixed(3)} />
                  <Metric label="Park Near" value={`${selectedRoute.park_near_pct.toFixed(1)}%`} />
                  <Metric label="Safety" value={selectedRoute.safety_score.toFixed(2)} />
        ...
        <section className="comparison-card">
          <div className="panel-header">
            ...
            <span className="panel-tag">Top alternatives side-by-side</span>
          </div>
                ...
                <tbody>
                  {routes.map((route, idx) => (
                    ...
                    >
                      ...
                      <td>{route.safety_score.toFixed(2)}</td>
                      <td>{route.turns}</td>
